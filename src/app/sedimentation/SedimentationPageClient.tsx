...
import type { SedimentationParams, RebarMeshInfo } from '@/lib/types';
import { calcSedimentation } from '@/lib/calc-well';
import { validateSedimentation, type ValidationError } from '@/lib/validate';
import { Field, NumField, Legend, ResetButton, SelectField, Section } from '@/components/FormControls';
import { ShareButton } from '@/components/ShareButton';
import { ViewerSkeleton } from '@/components/ViewerSkeleton';
import { getAvailableDiameters, COVER_TYPE_LABELS, lookupSedimentation } from '@/lib/tables';
...
const LEGEND_ITEMS = [
  ...
  { color: '#7F8C8D', label: '沉泥槽', opacity: 0.7 },
  { color: '#E67E22', label: '踏步/爬梯', opacity: 0.7 },
  { color: '#1ABC9C', label: '管道接口', opacity: 0.7 },
];

function makeDefault(): SedimentationParams {
  const avail = getAvailableDiameters('sedimentation');
  return {
    id: 'CN-Φ1000-3.0',
...
export function SedimentationPageClient() {
  const [params, setParams] = useState<SedimentationParams>(makeDefault);
  ...
  const result = useMemo(() => calcSedimentation(params), [params]);
  const { row, extra } = lookupSedimentation(params.diameter);

  function update<K extends keyof SedimentationParams>(key: K, value: SedimentationParams[K]) {
    const next = { ...params, [key]: value };
    setParams(next);
    setErrors(validateSedimentation(next));
  }
    ...
    <div className="flex flex-col xl:flex-row h-full">
      ...
      <aside className="w-full xl:w-80 shrink-0 bg-white border-r border-gray-100 overflow-y-auto">
        <div className="p-4 space-y-4">
          <div className="flex items-center gap-2 px-2 py-1.5 rounded-lg bg-sky-50 border border-sky-100 text-xs text-sky-700">
          ...
          <div className="space-y-3">
            ...
            <NumField label="井内径 Φ (mm)" value={params.diameter}
                      ...
                      min={800} max={2000} />
            <NumField label="井深 (mm, 不含沉泥槽)" value={params.depth}
      ...
      <aside className="w-full xl:w-72 shrink-0 bg-white border-l border-gray-100 overflow-y-auto">
        <div className="p-4 space-y-3">
          ...
          <div className="bg-gray-50/60 rounded-xl p-3 border border-gray-100">
            <h3 className="text-xs font-semibold text-gray-700 mb-2">06MS201-2 查表结果</h3>
            <div className="grid grid-cols-2 gap-1.5 text-[11px]">
              ...
              <span className="text-gray-500">沉泥槽深:</span><span className="text-gray-800 font-medium text-sky-600">{extra.sumpDepth}mm</span>
              <span className="text-gray-500">槽底板厚:</span><span className="text-gray-800">{extra.sumpBaseThickness}mm</span>
